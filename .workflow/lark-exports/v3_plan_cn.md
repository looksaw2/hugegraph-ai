...
1. Keys are lowercase (YAML convention)

键名小写（YAML 惯例）

1. LLM config nested by provider → role (natural grouping)

LLM 配置按 提供商 → 角色 嵌套（自然分组）

1. Merge priority / 合并优先级：`os.environ > config.yaml > pydantic defaults`
2. Environment variable mapping centralized in one place per config class

环境变量映射集中在每个配置类的一个位置

1. **V1 scope constraint / V1 范围约束：config.yaml changes require process restart to take effect**
...
1. Runtime `os.environ` / K8s Secret ONLY participate in `effective_config`, NEVER in `persisted_config`

运行时 `os.environ` / K8s Secret 仅参与 `effective_config`，绝不参与 `persisted_config`

1. If `.env` and `os.environ` both have the same key: runtime takes `os.environ`; migration write-source is only local `.env` or old YAML

如果 `.env` 和 `os.environ` 同时存在同名 key：运行时以 `os.environ` 为准；迁移写盘值仅来自本地 `.env` 或旧 YAML

1. After migration completes: **rename or backup `.env`** → `.env.bak` to prevent users from mistakenly treating it as an ongoing config source
