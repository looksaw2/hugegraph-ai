# Plan: .env → YAML → OmegaConf Nested Config Migration (V4 Contract-Strengthened)

# 方案：.env → YAML → OmegaConf 嵌套配置迁移（V4 契约加固版）

---

## What Changed from V3 to V4 / 从 V3 到 V4 的变更

**V3 review score / 审阅评分：7.8/10** — direction confirmed; implementation contracts need further strengthening.
方向正确；实现契约需进一步加固。

| # | Source / 来源 | Change / 变更 | Priority / 优先级 |
|---|--------------|---------------|-------------------|
| 1 | [飞书评论 P1] | `update_config()` API hardened: accept explicit patch dict only; enforce effective/env-sourced fields not written to YAML / `update_config()` API 加固：仅接受显式 patch dict；强制 effective/env-sourced 字段不写回 YAML | ‼️ P1 |
| 2 | [飞书评论 P2] | `PromptConfig`/`config_prompt.yaml` side-effect chain added to boundary analysis / `PromptConfig`/`config_prompt.yaml` 副作用链纳入边界分析 | ⚠️ P2 |
| 3 | [飞书评论 P2] | CI gate commands changed to real repo-executable paths / CI 门禁命令改为仓库实际可执行路径 | ⚠️ P2 |
| 4 | [飞书评论 P2] | Deployment docs changed to real-path acceptance checklist with verification grep / 部署文档改为真实路径级验收清单 + 验证 grep | ⚠️ P2 |
| 5 | [飞书评论 P2] | Unknown user key vs unmapped model field split into two separate strategies; unknown keys go to `_migration_unknown:` / unknown user key 与 unmapped model field 拆为两套策略；unknown key 归入 `_migration_unknown:` | ⚠️ P2 |
| 6 | [飞书评论 P2] | `.gitignore` rules scoped to `hugegraph-llm/`; add `config.example.yaml` / `.gitignore` 规则收窄到 `hugegraph-llm/`；添加 `config.example.yaml` | ⚠️ P2 |
| 7 | [飞书评论 P2] | Document title improved for readability / 文档标题可读性优化 | 🧹 P2 |
...
### Goal (Phase 2 — this plan, V4 contract-strengthened) / 目标（Phase 2 —— 本方案，V4 契约加固版）
...
### AFTER (Phase 2, V4 contract-strengthened) / 之后（Phase 2，V4 契约加固版）
...
### V4 new: PromptConfig / config_prompt.yaml side-effect chain / V4 新增：PromptConfig / config_prompt.yaml 副作用链

**Boundary analysis / 边界分析**：

The current `hugegraph_llm.config.__init__` creates `PromptConfig(llm_settings)` which calls `prompt.ensure_yaml_file_exists()` — reading/writing `hugegraph_llm/resources/demo/config_prompt.yaml`. This chain interacts with the config system because:
...
- `llm.language` is set via YAML/env override and PromptConfig selects/creates per-language prompt YAML
  `llm.language` 通过 YAML/env override 设置，PromptConfig 据此选择/创建不同语言的 prompt YAML
- In read-only container/package scenarios, PromptConfig might attempt to write inside the package
  在只读容器/只读 package 场景下，PromptConfig 可能尝试写 package 内部
- Current V3 tests cover read-only `config.yaml` but NOT prompt YAML read/write side effects
...
| Rule / 规则 | Detail / 详情 |
|------|--------|
| `config_prompt.yaml` MUST be explicitly classified | Options: user-writable config (same base dir as config.yaml) OR read-only resource (no auto-write) / 选项：用户可写配置（与 config.yaml 同基准目录）或只读资源（不自动写入） |
| If writable: path resolved relative to same base as `config.yaml` | Align config paths consistently / 统一配置路径基准 |
| If read-only: `ensure_yaml_file_exists()` MUST NOT write at startup | Fail-fast if missing, or bundle as package resource / 缺失时 fail-fast，或作为 package resource 打包 |
| Test coverage: `language=cn/en` × existing prompt yaml × read-only filesystem | Verifies prompt YAML behavior under all three combinations / 验证三种组合下的 prompt YAML 行为 |
...
- **V4 new / 新增**: Also map `PromptConfig`, `ensure_yaml_file_exists`, `config_prompt.yaml` references
  同时映射 `PromptConfig`、`ensure_yaml_file_exists`、`config_prompt.yaml` 引用
...
**Mapping coverage acceptance criteria (V4 strengthened) / 映射覆盖率验收标准（V4 加强）**：
...
**V4 new — Two-class split for unknown items / V4 新增 — unknown 分两类处理**：

| Category / 类别 | Definition / 定义 | Behavior / 行为 |
|---------|-----------|----------|
| **Model field unmapped** / 模型字段未映射 | A pydantic model field with no mapping entry and NOT classified as `intentional top-level` or `ignored-deprecated` | **CI fails** — must be resolved before merge / **CI 失败** —— 必须在合并前解决 |
| **Unknown user/legacy key** / 用户未知/旧 key | A key found in `.env` or Phase1 YAML that has no matching pydantic model field | **Preserved** in `_migration_unknown:` section; emits WARNING with source and key name / **保留**到 `_migration_unknown:` 节；输出 WARNING（含来源和 key 名） |

This prevents "reserved at top-level" from becoming a loophole that hides mapping audit failures. Unknown legacy keys are preserved without polluting the nested config structure:
这防止"保留在 top-level"成为掩盖映射审计失败的漏洞。未知旧 key 被保留但不污染嵌套配置结构：
...
| `update_section()` | **V4 hardened / 加固**: Accept explicit patch dict or `PersistedConfigPatch` ONLY — NOT a full effective model. Refuse to persist any value whose source is `env` or `runtime_patch`. / **仅接受显式 patch dict 或 `PersistedConfigPatch`** —— 不接受完整 effective model。拒绝持久化任何来源为 `env` 或 `runtime_patch` 的值 |
...
**V4 new — Field source tracking / V4 新增 — 字段来源追踪**：

```python
# ConfigManager maintains per-field source metadata
_field_source: dict[str, Literal["default", "yaml", "env", "runtime_patch"]]
```

This is the authoritative mechanism by which `update_section()` and `save()` reject env-sourced values. The field source is set once during `get_section_with_env_override()` and consulted by every write path.
...
**2.4 .env handling policy (V4 strengthened) / .env 处理策略（V4 加强）**
...
**3.3 `__init__` flow change (V4 hardened) / `__init__` 流程变更（V4 加强）**：
...
**V4 new — `update_config()` hardened API contract / V4 新增 — `update_config()` 加固 API 契约**：

The risk path identified in V3 review: singleton objects initialized from effective config may contain env secrets (`OPENAI_API_KEY`, `QDRANT_API_KEY`, `ADMIN_TOKEN`). If user modifies a non-sensitive field and calls `update_config()`, a naive full-dump implementation could write env secrets to `config.yaml`.
...
| V4 Rule / V4 规则 | Detail / 详情 |
|------|--------|
| `update_config()` MUST accept **explicit patch dict only** / 必须仅接受**显式 patch dict** | Example / 示例: `update_config({"openai_chat_language_model": "gpt-4.1"})` — NOT a full model dump / 不是全量 model dump |
| ConfigManager MUST track field source / ConfigManager 必须追踪字段来源 | `_field_source[field] ∈ {"default", "yaml", "env", "runtime_patch"}` |
| `update_section()` MUST reject effective model / MUST accept only `PersistedConfigPatch` or explicit dict / `update_section()` 必须拒绝 effective model，仅接受 `PersistedConfigPatch` 或显式 dict |
| Env-sourced field values MUST be excluded from all write paths / env 来源字段值必须在所有写入路径中排除 | Field source check before every `cfg[key] = value` assignment / 每次赋值前检查字段来源 |

Test requirement / 测试要求:
- YAML has no `api_key`, env has `OPENAI_API_KEY=env-secret`
- User changes only `language_model` then saves
- Assert `config.yaml` does NOT contain `env-secret`
...
- `update_env()` → `update_config()`: persist to YAML instead of `.env`. **Accepts explicit patch dict only (V4 hardened).** / 持久化到 YAML 而非 `.env`。**仅接受显式 patch dict（V4 加固）。**
...
**3.6 Three-format migration path (V4 strengthened) / 三格式迁移路径（V4 加强）**：
...
**Multi-format conflict resolution decision table (V4 strengthened) / 多格式冲突决策表（V4 加强）**：
...
| **Unknown user/legacy keys (V4 refined)** / **用户未知/旧 key（V4 细化）** | **Preserve in `_migration_unknown:` section** with source annotation; emit trackable **warning** with field name and source / **保留到 `_migration_unknown:` 节**并标注来源；输出可追踪 **warning**（含字段名和来源） |
| **Unmapped model fields (V4 refined)** / **模型字段未映射（V4 细化）** | **CI fails** unless classified as `intentional top-level` / `ignored-deprecated` — NOT silently preserved at top-level / **CI 失败**，除非归类为 `intentional top-level` / `ignored-deprecated` —— 不静默保留到 top-level |
...
detect old format / 检测旧格式
  ...
  → separate known model fields vs unknown legacy keys / 分离已知模型字段与未知旧 key
    → known fields → nested section / 已知字段 → 嵌套节
    → unknown keys → _migration_unknown section (V4: preserved, not scattered at top-level) / 未知 key → _migration_unknown 节（V4：保留，不散落 top-level）
...
**Side-effect-free validation contract (V4) / 无副作用验证契约（V4）**：
...
- **V4 new / 新增**: PromptConfig initialization and `config_prompt.yaml` write side effects / PromptConfig 初始化和 `config_prompt.yaml` 写入副作用
...
**Env var resolution contract — ordered (V4 hardened, P1) / 环境变量解析契约 —— 有序（V4 加强，P1）**：
...
|------|--------|---------|
| `config/__init__.py` | Initialize `ConfigManager(sections={...})` before config singletons. **V4: audit PromptConfig side effects** — ensure `config_prompt.yaml` path aligns with `config.yaml` base dir / 在配置单例之前初始化 `ConfigManager`。**V4：审计 PromptConfig 副作用** —— 确保 `config_prompt.yaml` 路径与 `config.yaml` 基准目录一致 | Singleton must exist before any config object calls it / 单例必须在任何配置对象调用它之前存在 |
| `config/generate.py` | 4 × `generate_env()` → `generate_yaml()` | API surface consistency / API 表面一致性 |
| `demo/rag_demo/configs_block.py` | 6 × `update_env()` → `update_config()`; remove `dotenv_values` import; replace direct `.env` reads with config object attributes. **V4: ensure `update_config()` calls pass explicit patch dict, not full model dump** / 移除 `dotenv_values` 导入；用配置对象属性替换直接 `.env` 读取。**V4：确保 `update_config()` 传入显式 patch dict，非全量 model dump** | Primary consumer / 主要消费者 |
| `pyproject.toml` | Add / 添加 `omegaconf~=2.3` | New dependency / 新依赖 |
| `.gitignore` | **V4 scoped / 收窄**: `hugegraph-llm/config.yaml`, `hugegraph-llm/config.yaml.bak`, `hugegraph-llm/.env.bak` | Sensitive data + migration backups / 敏感数据 + 迁移备份 |
| **V4 new / 新增** `config.example.yaml` | Commit a non-sensitive example YAML with placeholder values — **no real tokens/keys** / 提交非敏感示例 YAML，占位值 —— **不含真实 token/key** | User onboarding: documents nested YAML structure without requiring migration / 用户入门：无需迁移即可了解嵌套 YAML 结构 |
| `config.md` | Rewrite docs for nested structure + priority; document that config changes require restart; **V4: document `config_prompt.yaml` lifecycle** / 重写文档以反映嵌套结构 + 优先级；文档说明配置修改需重启；**V4：说明 `config_prompt.yaml` 生命周期** | User-facing documentation / 面向用户的文档 |
...
| **Unmapped field audit (V4)** / **未映射字段审计（V4）** | Assert zero unreviewed unmapped fields per config class; every field classified as mapped / intentional top-level / ignored-deprecated; **V4: assert model-field-unmapped ≠ user-legacy-key — two separate checks** / 断言每个 config class 无不经审查的 unmapped field；**V4：断言 model-field-unmapped ≠ user-legacy-key —— 两项独立检查** |
...
| **V4 P1**: partial field update does NOT leak env secret to YAML / 部分字段更新不泄露 env secret 到 YAML | YAML has no `api_key`, env has `OPENAI_API_KEY=env-secret`, user changes `language_model` only → `config.yaml` does NOT contain `env-secret` |
...
| Gradio apply callbacks write to YAML not .env / Gradio 应用回调写入 YAML 而非 .env | UI config changes persist to `config.yaml`; **V4: verify patch-based write, not full dump** / UI 配置修改持久化到 `config.yaml`；**V4：验证 patch-based 写入，非全量 dump** |
...
| **Read-only config.yaml or read-only directory (V4)** / **只读 config.yaml 或只读目录（V4）** | Startup read succeeds; only explicit save/update fails with clear error; **V4 extended: prompt YAML read-only scenario also covered** / 启动读取应可用；只有显式 save/update 才失败并报清晰错误；**V4 扩展：同时覆盖 prompt YAML 只读场景** |
| **Secret masking in logs/errors/diffs (V4)** / **日志/错误/diff 中密钥脱敏（V4）** | No plaintext secret in migration failure, env override, save, or check_config output / 迁移失败、env 覆盖、save、check_config 输出中均不出现明文密钥 |
| **Migration failure does not corrupt config.yaml (V4)** / **迁移失败不损坏 config.yaml（V4）** | Validation failure aborts without generating or overwriting config.yaml / 校验失败终止，不生成或覆盖 config.yaml |
| **Multi-format conflict resolution (V4)** / **多格式冲突解决（V4）** | Phase1 YAML + .env coexistence: YAML wins, .env diffs get warnings / Phase1 YAML + .env 共存：YAML 优先，.env 差异打 warning |
| **V4 new: Unknown legacy key → _migration_unknown (V4)** / **未知旧 key → _migration_unknown（V4 新增）** | .env keys with no pydantic field go to `_migration_unknown:` section, NOT scattered at YAML top-level / .env 中无 pydantic 字段对应的 key 归入 `_migration_unknown:` 节，不散落 YAML top-level |
| **V4 new: PromptConfig side effects (V4)** / **PromptConfig 副作用（V4 新增）** | `language=cn/en` + existing prompt yaml + read-only filesystem / `language=cn/en` + 已有 prompt yaml + 只读文件系统 |

**7.3 CI Integration (V4 — real executable paths) / CI 集成（V4 — 真实可执行路径）**：

Tests that MUST run in CI (every PR) / 必须在 CI 中运行的测试（每次 PR）：

| CI Gate / CI 门禁 | Command / 命令 |
|---------|---------|
| **Migration + env priority + secret not persisted + deprecated wrapper + update_config patch** | `cd hugegraph-llm && SKIP_EXTERNAL_SERVICES=true uv run pytest src/tests/config/ -k "migration or env or secret or deprecated or cwd or path or readonly or unmapped or coverage or mask or prompt" -v --tb=short` |
| Lint / format check | `uv run ruff format --check . && uv run ruff check .` |
| Config path CWD-independence | Covered by `-k "cwd or path"` above / 包含在上述 filter 中 |
| Read-only filesystem scenario (incl. prompt YAML) | Covered by `-k "readonly or prompt"` above / 包含在上述 filter 中 |
| Unmapped field audit (incl. unknown→_migration_unknown) | Covered by `-k "unmapped or coverage"` above / 包含在上述 filter 中 |
| Secret masking verification | Covered by `-k "mask or secret"` above / 包含在上述 filter 中 |
| **V4 new: update_config() patch-based API enforcement** | Single pytest line; `-k` already covers via "secret" gate / 单行 pytest；`-k` 已通过 "secret" 覆盖 |
| **V4 new: Real-link test if Gradio/API callback touched** | Verify `ConfigManager → BaseConfig → UI/API write` chain end-to-end, not just pure helper functions / 验证 `ConfigManager → BaseConfig → UI/API write` 链路端到端 |

---

### Task 8: Deployment Documentation Update (V4 — real-path checklist) / 任务 8：部署文档更新（V4 — 真实路径验收清单） `[Priority: Medium / 中]` (Depends on: Task 6)

**Goal / 目标**：Update all deployment entry points with a verifiable path-level checklist.
以可验证的路径级清单更新所有部署入口。

**Real-path inventory / 真实文件清单**：

| Document / 文档 | Update / 更新内容 | Verification / 验证 |
|---------|----------|------|
| `README.md` | Remove `.env` references; add `config.yaml` setup instructions / 移除 `.env` 引用；添加 `config.yaml` 配置说明 | `rg "\.env\|dotenv\|generate_env\|update_env\|check_env" README.md` → must return zero / 必须返回零结果 |
| `hugegraph-llm/README.md` | Same audit / 同样审计 | `rg "\.env\|dotenv\|generate_env\|update_env\|check_env" hugegraph-llm/README.md` → zero / 零 |
| `hugegraph-llm/config.md` | Nested structure + priority + restart + PromptConfig lifecycle / 嵌套结构 + 优先级 + 重启 + PromptConfig 生命周期 | Manual review / 人工审核 |
| `docker/env.template` | Full `config.yaml` template with comments | Already done / 已完成 |
| `docker/docker-compose.yml` | Add `config.yaml` volume mount; remove `.env` file references | `rg "\.env" docker/` → zero for runtime .env refs / 运行时 .env 引用应为零 |
| `docker/docker-compose-network.yml` | Same audit | Same |
| `docker/docker-compose-llm.yml` | Updated comments | Check secret-related env var comments / 检查 secret 相关 env var 注释 |
| `docker/charts/hg-llm/values.yaml` | Update ConfigMap to use nested YAML; document env Secret priority | Manual review / 人工审核 |
| `docker/charts/hg-llm/Chart.yaml` | Audit for .env refs | `rg "\.env" docker/charts/` → zero / 零 |
| `docker/charts/hg-llm/templates/*` | Remove `.env` ConfigMap refs; add `config.yaml` ConfigMap (if config/env templates exist) / 移除 `.env` ConfigMap 引用；添加 `config.yaml` ConfigMap（如果存在 config/env 模板） | `rg "\.env" docker/charts/hg-llm/templates/` → zero / 零 |
| `.github/workflows/hugegraph-llm.yml` | **V4: config test gate with real executable paths** | Verify `pytest src/tests/config/` with correct `-k` filter |
| Upgrade guide (new) / 升级指南（新增） | Step-by-step for old `.env` users: backup → migrate → verify → rollback procedure / 旧 `.env` 用户的分步指南：备份 → 迁移 → 验证 → 回滚流程 | N/A |
| **V4 new** `hugegraph-llm/config.example.yaml` | Commit non-sensitive example with placeholder values and inline comments / 提交非敏感示例含占位值和内联注释 | Manual review / 人工审核 |

**Global verification command / 全局验证命令**：

```bash
# Run from repo root: must return zero for all deployment-related paths
rg "\.env|dotenv|generate_env|update_env|check_env" \
  README.md hugegraph-llm/README.md hugegraph-llm/config.md \
  docker/ .github/ \
  --ignore-case
# Expected: zero results for runtime .env references
# (allowed: migration docs describing the .env → YAML transition)
```

**Key messaging for deployment docs / 部署文档的关键信息**：

- Non-sensitive config: use `config.yaml` (mount file or ConfigMap). Reference `config.example.yaml` for structure.
  非敏感配置：使用 `config.yaml`（挂载文件或 ConfigMap）。参考 `config.example.yaml` 了解结构。
- Sensitive config (secrets): continue using env / K8s Secret, which takes priority over YAML
  敏感配置（密钥）：继续使用 env / K8s Secret，优先级高于 YAML
- Config changes require restart; V1 does not support hot-reload
  配置修改需重启；V1 不支持热加载
- Old `.env` users: follow the upgrade guide for one-time migration
  旧 `.env` 用户：按升级指南完成一次性迁移
- **V4 new**: `config_prompt.yaml` lifecycle documented — user-writable (aligned with config.yaml base dir) or read-only resource
...
## 5. Risk Assessment (V4 — contract-strengthened + PromptConfig boundary) / 风险评估（V4 —— 契约加固 + PromptConfig 边界）
...
| Missing entry in `_flat_to_nested_mapping` / `_flat_to_nested_mapping` 中缺少条目 | Medium / 中 | **V4: Mandatory field classification audit** + **two-class split**: model-field-unmapped → CI fail; user-legacy-key → `_migration_unknown:` / **强制字段分类审计** + **两类拆分**：model-field-unmapped → CI 失败；user-legacy-key → `_migration_unknown:` |
...
| **V4 P1: Env secrets written back to config.yaml via `update_config()` full dump** / **V4 P1：通过 `update_config()` 全量 dump 将 env secret 写回 config.yaml** | High / 高 | `update_config()` accepts **explicit patch dict ONLY**; ConfigManager tracks `_field_source[field] ∈ {default, yaml, env, runtime_patch}`; `save()` excluaeds all `env`-source fields; `update_section()` rejects effective model inputs; **dedicated test: YAML no api_key + env has secret + partial field change → config.yaml does NOT contain secret** / `update_config()` **仅接受显式 patch dict**；ConfigManager 追踪字段来源；`save()` 排除所有 `env` 来源字段；`update_section()` 拒绝 effective model 输入；**专项测试：YAML 无 api_key + env 有 secret + 部分字段修改 → config.yaml 不含 secret** |
| **Sensitive info leaked in logs/errors/diffs (V4)** / **敏感信息在日志/错误/diff 中泄露（V4）** | High / 高 | Fields with `api_key`/`token`/`password`/`pwd`/`secret` in name are uniformly masked; error messages show field path + source but NOT raw value; `check_config()` / diff output shows `***` or `changed`/`not changed`; test coverage for: migration failure, env override, save, check_config, **partial update_config()** logs contain no plaintext secrets / 字段名含 `api_key`/`token`/`password`/`pwd`/`secret` 的统一脱敏；错误信息显示字段路径和来源但不显示原值；`check_config()` / diff 输出仅显示 `***` 或 `changed`/`not changed`；测试覆盖迁移失败、env 覆盖、save、check_config、**partial update_config()** 日志中不出现明文密钥 |
| **V4 new: PromptConfig writes to package resource in read-only deployment** / **V4 新增：PromptConfig 在只读部署中写入 package resource** | Medium / 中 | Classify `config_prompt.yaml` as user-writable config OR read-only resource; if writable, align path with `config.yaml` base dir; if read-only, forbid auto-write at startup; test `language=cn/en` + existing prompt yaml + read-only filesystem / 将 `config_prompt.yaml` 归类为用户可写配置或只读资源；若可写，路径与 `config.yaml` 基准目录对齐；若只读，禁止启动时自动写入；测试 `language=cn/en` + 已有 prompt yaml + 只读文件系统 |
| **Migration partially writes corrupt config.yaml (V4)** / **迁移部分写入损坏的 config.yaml（V4）** | High / 高 | Side-effect-free validation via `model_validate`/`TypeAdapter` (no `__init__`, **no PromptConfig init**); validation failure aborts without touching `config.yaml`; atomic write (write temp + rename) / 无副作用校验通过 `model_validate`/`TypeAdapter`（不经过 `__init__`，**不经过 PromptConfig init**）；校验失败终止，不触及 `config.yaml`；原子写入（写临时文件 + 重命名） |
| Phase 0/1 YAML not auto-detected / Phase 0/1 YAML 未自动检测 | Medium / 中 | Explicit format detection + **V4 multi-format conflict decision table** (Task 3.6); do not rely on OmegaConf to guess the format / 显式格式检测 + **V4 多格式冲突决策表**（Task 3.6）；不依赖 OmegaConf 猜测格式 |
| `.env` mistakenly treated as ongoing config source / `.env` 被误认为持续配置源 | Medium / 中 | **V4: Auto-backup `.env` → `.env.bak` after migration**; `.env` never loaded into `os.environ`; clear runtime boundary between `.env` (migration-only) and `os.environ` (runtime-only) / **V4：迁移后自动备份 `.env` → `.env.bak`**；`.env` 绝不加载到 `os.environ`；`.env`（仅迁移）与 `os.environ`（仅运行时）之间的清晰边界 |
| Config path depends on CWD / 配置路径依赖 CWD | Low / 低 | Resolve `config.yaml` path relative to a fixed base directory, not `os.getcwd()`; **V4: same applies to config_prompt.yaml and config.example.yaml** / 相对于固定基准目录解析 `config.yaml` 路径，而非 `os.getcwd()`；**V4：config_prompt.yaml 和 config.example.yaml 同理** |
| Env alias resolution non-deterministic / 环境变量别名解析不确定 | Medium / 中 | **V4: Ordered resolution contract** — field-level > provider-level; stored as list/tuple not set; empty string does not clear YAML value; fail-fast on type mismatch / **V4：有序解析契约** —— 字段级 > 提供商级；存储为 list/tuple 非 set；空字符串不清除 YAML 值；类型不匹配 fail-fast |
| **V4 new: Deployment doc references stale paths / V4 新增：部署文档引用过期路径** | Low / 低 | **V4: Real-path acceptance checklist** (Task 8) with verification `rg` command; audit `README.md`, `hugegraph-llm/README.md`, `docker/`, `.github/`, `docker/charts/` / **V4：真实路径验收清单**（Task 8）+ 验证 `rg` 命令；审计所有部署相关路径 |

**Core principle / 核心原则**：Configuration errors should be exposed as early and as loudly as possible. Silent fallback to defaults risks connecting to wrong services or using wrong credentials in production. Secrets must never appear in plaintext in any output channel (files, logs, errors, diffs). **V4 addition: Config write paths must be patch-based, not full-dump — a single non-sensitive field change must not carry env secrets to disk.**
配置错误应尽可能早、尽可能大声地暴露。静默回退到默认值可能导致在生产环境中连接到错误的服务或使用错误的凭据。密钥绝不在任何输出渠道（文件、日志、错误、diff）中以明文出现。**V4 新增：配置写入路径必须基于 patch 而非全量 dump —— 单个非敏感字段变更不得携带 env secret 落盘。**
...
| **Mapping coverage audit + field classification (V4 strengthened)** / **映射覆盖率审计 + 字段分类（V4 加强）** | Included / 包含 |
| **Two-class split: model-unmapped (CI fail) vs user-legacy-key (_migration_unknown) (V4 new)** / **两类拆分：model-unmapped（CI 失败）vs user-legacy-key（_migration_unknown）（V4 新增）** | Included / 包含 |
| **.env vs os.environ vs K8s Secret boundary (V4)** / **.env vs os.environ vs K8s Secret 边界（V4）** | Included / 包含 |
| **No auto-persist on `__init__` (V4)** / **`__init__` 不自动持久化（V4）** | Included / 包含 |
| **Patch-based `update_config()` API — no full model dump (V4 new)** / **Patch-based `update_config()` API —— 不全量 model dump（V4 新增）** | Included / 包含 |
| **Field source tracking: default / yaml / env / runtime_patch (V4 new)** / **字段来源追踪：default / yaml / env / runtime_patch（V4 新增）** | Included / 包含 |
| **Side-effect-free migration validation (V4)** / **无副作用迁移验证（V4）** | Included / 包含 |
| **Multi-format conflict decision table (V4)** / **多格式冲突决策表（V4）** | Included / 包含 |
| **Ordered env var resolution contract (V4)** / **有序环境变量解析契约（V4）** | Included / 包含 |
| **Full sensitive info masking (V4)** / **完整敏感信息脱敏（V4）** | Included / 包含 |
| **PromptConfig boundary analysis (V4 new)** / **PromptConfig 边界分析（V4 新增）** | Included / 包含 |
| **Deployment doc update with real-path checklist (V4 strengthened)** / **部署文档更新 + 真实路径清单（V4 加强）** | Included / 包含 |
| **config.example.yaml (V4 new)** / **config.example.yaml（V4 新增）** | Included / 包含 |
| **CI gates with real executable paths (V4 strengthened)** / **CI 门禁 + 真实可执行路径（V4 加强）** | Included / 包含 |
| **Read-only scenario tests incl. prompt YAML (V4 strengthened)** / **只读场景测试含 prompt YAML（V4 加强）** | Included / 包含 |
...
### Hardened Contracts (V4 — strengthened from V3) / 强化契约（V4 —— 自 V3 加强）

| Priority / 优先级 | Contract / 契约 | V3→V4 Change / V3→V4 变更 | Enforced In / 执行位置 |
|------|----------|------|------------|
| ‼️ P0 | `__init__` MUST NOT auto-persist env override values to YAML / `__init__` 不得将 env 覆盖值自动写回 YAML | No change (already P0) / 无变更（已是 P0） | Task 3.3, §5 Risk |
| ‼️ P0 | `.env` (migration-only) vs runtime `os.environ` vs K8s Secret MUST have hard boundaries / `.env`（仅迁移）、运行时 `os.environ`、K8s Secret 必须有硬边界 | No change (already P0) / 无变更（已是 P0） | Task 2.4, §2 Architecture |
| ‼️ P0 | Env alias resolution MUST be ordered: field-level > provider-level; empty string does not clear YAML / 环境变量别名解析必须有序：字段级 > 提供商级；空字符串不清除 YAML | No change (already P0) / 无变更（已是 P0） | Task 4, Task 7.2 |
| ‼️ **V4 P1 (NEW)** | **`update_config()` MUST accept explicit patch dict only — NOT full model dump; ConfigManager MUST track field source; `save()` MUST exclude all env-sourced values** / **`update_config()` 必须仅接受显式 patch dict —— 不全量 model dump；ConfigManager 必须追踪字段来源；`save()` 必须排除所有 env 来源值** | **New in V4 — hardened from review feedback** / **V4 新增 —— 基于审阅反馈加固** | Task 3.3, Task 7.2, §5 Risk |
| ⚠️ P1 | Three-format migration MUST have conflict decision table + side-effect-free validation / 三格式迁移必须有冲突决策表 + 无副作用验证 | No change / 无变更 | Task 3.6 |
| ⚠️ P1 | Mapping coverage MUST be audited: every field classified; CI asserts zero unreviewed unmapped / 映射覆盖率必须审计：每个字段归类；CI 断言零未审查 unmapped | **V4: split model-unmapped (CI fail) vs user-legacy (_migration_unknown)** / **V4：拆分为 model-unmapped（CI 失败）vs user-legacy（_migration_unknown）** | Task 2.1, Task 7.1, Task 7.3 |
| ⚠️ P1 | Sensitive fields MUST be masked in all output: logs, errors, diffs, check_config, **partial update_config()** / 敏感字段必须在所有输出中脱敏：日志、错误、diff、check_config、**partial update_config()** | **V4 extended: include partial update_config() path** / **V4 扩展：包含 partial update_config() 路径** | §5 Risk, Task 7.2 |
| ⚠️ **V4 P2 (NEW)** | **Deployment docs MUST use real-path acceptance checklist with verification rg command** / **部署文档必须使用真实路径验收清单 + 验证 rg 命令** | **New in V4 — converted from P2 review feedback** / **V4 新增 —— 从审阅 P2 反馈转换** | Task 8 |
| ⚠️ **V4 P2 (NEW)** | **CI gate commands MUST be real repo-executable paths (not placeholders)** / **CI 门禁命令必须是仓库实际可执行路径（非占位符）** | **New in V4 — converted from P2 review feedback** / **V4 新增 —— 从审阅 P2 反馈转换** | Task 7.3 |
| ⚠️ **V4 P2 (NEW)** | **PromptConfig/config_prompt.yaml side-effect chain MUST be classified and documented** / **PromptConfig/config_prompt.yaml 副作用链必须被归类并文档化** | **New in V4 — converted from P2 review feedback** / **V4 新增 —— 从审阅 P2 反馈转换** | §2 Architecture, Task 6, §5 Risk |

---

## Appendix: V3 → V4 Change Traceability / 附录：V3 → V4 变更追溯

| Feishu Comment # | Issue / 问题 | V4 Sections Modified / V4 修改的章节 |
|-----------------|-------------|----------------------------------|
| P1 (inline) | `update_config()` full dump risks env secret leak | §5 Risk (new risk row), Task 3.3 (V4 new API contract), Task 7.2 (new test), §7 Hardened Contracts (new P1 row) |
| P2 (inline) | PromptConfig/config_prompt.yaml side effects | §2 Architecture (new PromptConfig sub-section), Task 1 (extended grep scope), Task 6 (config/__init__.py note), Task 7.2 (new test), §5 Risk (new risk row) |
| P2 (inline) | CI command placeholder paths | Task 7.3 (real executable commands), Task 8 (verification grep) |
| P2 (inline) | Deployment doc missing real-path checklist | Task 8 (real-path inventory + verification command), §7 Hardened Contracts (new P2) |
| P2 (inline) | Unknown user key vs unmapped model field conflated | §2.1 (two-class split), Task 3.6 (decision table), Task 7.2 (new test), §7 Scope Summary |
| P2 (inline) | .gitignore too broad + need config.example.yaml | Task 6 (.gitignore scoping + new example file), Task 8 (config.example.yaml entry) |
| P2 (title) | Title readability | Document title |
| Overall review | 7.8/10, direction confirmed | §0 (What Changed from V3 to V4 summary table), §7 (updated Hardened Contracts) |
