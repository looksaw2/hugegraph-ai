...
### 2.1 P0: `__init__` No Longer Auto-Persists (Gap 3)

**File**: `base_config.py:397-405`

**Before**: `BaseConfig.__init__` called `cfg_mgr.update_section()` + `cfg_mgr.save()` on every construction, risking env secrets being written to disk.

**After**: `__init__` only logs initialization. No disk I/O.
