...
- **6. 计划与状态分离 (Plan/Status Separation)**: 需求、设计、方案和飞书计划文档只表达目标、实现方法、代码路径、约束和验收标准，不用 `[x]` / `[ ]` 表达完成状态。复选框仅用于 `.workflow/{feature_name}/tasks.md` 这类执行任务清单；只有执行阶段完成并有代码 diff、测试结果或验收记录后，才能标记为 `[x]`。

- **7. 飞书文档可读性 (Lark Document Readability)**: 面向飞书阅读的方案文档避免大段 Markdown 表格、复选框和删除线；表格横线或 checkbox 影响阅读时，改用“标题 + 普通列表”。不要把计划项写成已完成清单。
