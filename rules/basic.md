...
#### 0.【必须】区分计划文档与执行状态 (Mandatory: Separate Plans From Execution Status)

在撰写或更新方案文档、设计文档、飞书文档、README 或规则文档时，必须明确区分“计划目标”和“已完成状态”。

- 普通计划文档只写目标、实现方法、代码路径、任务拆分和验收标准，不使用 `[x]` / `[ ]` 复选框表达状态。
- `[x]` 只允许出现在真实执行阶段的 `.workflow/{feature_name}/tasks.md` 任务清单或执行日志中，且必须有对应代码 diff、测试结果或验收记录支撑。
- 飞书文档中不要把计划内容写成 checkbox；checkbox 会渲染成完成态或划线效果，容易误导为已经完成。
- 面向飞书阅读的方案文档尽量少用 Markdown 表格。若表格分隔线影响阅读，改用标题 + 普通列表。
- 不得使用 Markdown 删除线表达计划项状态。
- 不得把“计划提供”“拟实现”“拟新增”的能力写成“已实现”“已通过”“已完成”。
