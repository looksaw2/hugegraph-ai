...
- **格式**:
  - 所有任务项都是一个未选中的复选框 `[ ]`。
  - 复选框格式只适用于 `.workflow/{feature_name}/tasks.md` 这种执行任务清单，不适用于需求文档、设计文档、方案文档、飞书计划文档或 README。
  - 规划阶段严禁输出 `[x]`。任务只有进入执行阶段，并且代码、测试、日志或验收记录已经完成后，才能由执行流程标记为 `[x]`。
  - 若输出的是 Feishu-facing 方案、迁移计划、主线改造计划或 README，必须使用普通标题和列表表达“拟新增 / 拟修改 / 验收标准”，不得套用本节的 checkbox 示例。
  - 顶层任务使用整数编号（`1.`, `2.`）。
  - 子任务使用小数编号（`1.1.`, `1.2.`）。
  - 每个任务项需包含：
...
注意：以下示例仅限 `.workflow/{feature_name}/tasks.md` 执行任务清单，不能复制到飞书方案文档、README、设计文档或迁移计划中。
...
- 在普通方案文档或飞书文档中使用 `[x]` / `[ ]` 造成“已完成”或“划线”效果。
